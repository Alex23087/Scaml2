...
let () =
  ...
  let fname = args.(1) in
  let should_print_expr =
    if Array.length args > 2 then String.( = ) args.(2) "--print-expr"
    else false
  in
  ...
  | Ok toks ->
      ...
      |> Interpreter.eval (Core.Filename.dirname fname)
      (* |> (fun (v, l) -> "(" ^ Val.to_string v ^ ", " ^ Sexp.to_string_hum (Lbl.sexp_of_t l) ^ ")")
         |> print_endline *)
      |>
      fun _ -> ()
  | Error err ->
      Out_channel.print_endline ("lexer error: " ^ Lexer.error_to_string err)
